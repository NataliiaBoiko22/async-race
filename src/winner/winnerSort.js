...
import { getCars } from "../store/api-servise";

export const createSortedWinnerList = async (winners) => {
  console.log(winners);
  let blocks = document.querySelectorAll(".winner_row");
  ...
  winners.forEach((winner) => {
    for (let racer of allCars) {
      if (racer.id == winner.id) {
        let sortedWinnerToTheList = document.createElement("tr");
        sortedWinnerToTheList.classList.add("winner_row");
        sortedWinnerToTheList.id = `winner_row-${racer.id}`;
        sortedWinnerToTheList.innerHTML = `
  
          <td class="number"></td>
          <td class="car_mini">
          <svg class="car_img_mini"
          fill="${racer.color}"  version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 1000 1000" enable-background="new 0 0 1000 1000" xml:space="preserve">
          ...
          <td class="car_mini_name">${racer.name}</td>
          <td class="car_mini_wins">${winner.wins}</td>
          <td class="car_mini_time">${winner.time}</td>
          `;

        table.append(sortedWinnerToTheList);
      }
    }
    countWinner();
    btnWinnersDesabling();
